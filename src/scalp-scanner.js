...
export async function runScalpScan() {
  // Use oi-movers (15m) as primary taker-flow proxy
  let oiMovers;
  try {
    oiMovers = await apiGet('scan/oi-movers', {
  ...
  if (!Array.isArray(oiMovers) || !oiMovers.length) return [];

  // Fetch perp snapshot for each candidate to get taker ratio
  const candidates = await Promise.allSettled(
    oiMovers.slice(0, 20).map(async (m) => {
